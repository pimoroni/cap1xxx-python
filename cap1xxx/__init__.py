...
class Cap1xxx:
    supported: ClassVar[list[int]] = [PID_CAP1208, PID_CAP1188, PID_CAP1166]
    number_of_inputs = 8
    number_of_leds = 8
    ...
    def _trigger_handler(self, channel, event):
        ...
        if callable(self.handlers[event][channel]):
            try:
                self.handlers[event][channel](
                    CapTouchEvent(channel, event, self.input_delta[channel])
                )
            except TypeError:
...
class Cap1208(Cap1xxx):
    supported: ClassVar[list[int]] = [PID_CAP1208]


class Cap1188(Cap1xxxLeds):
    number_of_leds = 8
    supported: ClassVar[list[int]] = [PID_CAP1188]


class Cap1166(Cap1xxxLeds):
    number_of_inputs = 6
    number_of_leds = 6
    supported: ClassVar[list[int]] = [PID_CAP1166]
